...
推荐系统
1. 以电影推荐场景为例：
    - 推荐系统要解决的问题，去查找那些没有被打分的电影，并尝试去预测这些电影的评价星级。
    - 推荐系统是要设计一个算法，能自动为我们填补缺失值的算法。这样可以看出该用户还有哪些电影没有看过，并推荐新电影给用户。
    ![alt](./mlpics/rs1.png)

1. 基于内容的推荐算法（Content-based recommender system）
    - 算法目标：对于每个用户 $j$, 学习一个参数 $\theta^{(j)} \in \mathbb R^3$. 预测用户 $j$ 给电影 $i$ 的评分 $(\theta^{(j)})^Tx^{(i)}$
    - $n_u$ - 用户数 
    - $n_m$ - 电影数 
    - $r(i,j)=1$ - 如果用户 $j$ 给电影 $i$ 投了票
    - $y^{i,j}$ - 用户 $j$ 给电影 $i$ 的评分
    - $\theta^{(j)}$ - 用户 $j$ 的参数向量
    - $x^{(i)}$ - 电影 $i$ 的特征向量
    - $m^{(j)}$ - 用户 $j$ 评过分的电影数量
    - 学习 $\theta^{(j)}, \theta^{(j)} \in \mathbb R^{n+1}$。为用户 $j$ 的特征向量
    
    $$
    \underset{\theta^{(j)}}{min} \frac{1}{2m^{(j)}} 
    \sum_{i:r(i,j)=1} \Big((\theta^{(j)})^Tx^{(i)}-y^{(i,j)}\Big)^2 + 
    \frac{\lambda}{2m^{(j)}} \sum_{k=1}^{n}(\theta_k^{(j)})^2
    $$

    - 为了简化公式计算，去掉常数项 $m^{(j)}$, 对计算结果不影响

    $$
    \underset{\theta^{(j)}}{min} \frac{1}{2} 
    \sum_{i:r(i,j)=1} \Big((\theta^{(j)})^Tx^{(i)}-y^{(i,j)}\Big)^2 + 
    \frac{\lambda}{2} \sum_{k=1}^{n}(\theta_k^{(j)})^2
    $$

    - 为了学习其他用户们的特征参数 $\theta^{(1)},\theta^{(2)},...,\theta^{(n_u)} :$,需要最优化的目标函数

    $$
    \underset{\theta^{(1)},\theta^{(2)},...,\theta^{(n_u)}}{min} \frac{1}{2} 
    \sum_{j=1}^{n_u}
    \sum_{i:r(i,j)=1} \Big((\theta^{(j)})^Tx^{(i)}-y^{(i,j)}\Big)^2 + 
    \frac{\lambda}{2} 
    \sum_{j=1}^{n_u}
    ...
    \sum_{i:r(i,j)=1} \Big((\theta^{(j)})^Tx^{(i)}-y^{(i,j)}\Big) x_k^{(i)},(for \ k=0)
    $$

    $$
    \theta_k^{(j)}:=\theta_k^{(j)}-\alpha \bigg(
...
1. 协同过滤（Collaborative filtering）
    - 基于内容的过滤是找到相似的影片来推荐（已知电影特征，找用户特征，即给出 $x^{(1)},...,x^{(n_m)}$ (和电影评分)，可以估计 $\theta^{(1)},...,\theta^{(n_u)}$），协同过滤是找到相似的用户相互推荐（已知用户特征，找电影特征，即给出 $\theta^{(1)},...,\theta^{(n_u)}$，可以估计 $x^{(1)},...,x^{(n_m)}$）。

    - 算法公式：
        - 已知 $\theta^{(1)},...,\theta^{(n_u)}$, 学习 $x^{(i)}$：

        $$
        \underset{x^{(i)}}{min} \frac{1}{2} 
        \sum_{j:r(i,j)=1} \Big((\theta^{(j)})^Tx^{(i)}-y^{(i,j)}\Big)^2 + 
        \frac{\lambda}{2} \sum_{k=1}^{n}(x_k^{(i)})^2
        $$

        - 已知 $\theta^{(1)},...,\theta^{(n_u)}$, 学习 $x^{(1)},...,x^{(n_m)}$：

        $$
        \underset{x^{(1)},x^{(2)},...,x^{(n_m)}}{min} \frac{1}{2} 
        \sum_{i=1}^{n_m}
        \sum_{j:r(i,j)=1} \Big((\theta^{(j)})^Tx^{(i)}-y^{(i,j)}\Big)^2 + 
        \frac{\lambda}{2} 
        \sum_{i=1}^{n_m}
        ...
        - 规划一个协同过滤的问题，从随机猜一个 $\theta$ 开始，不断迭代，不断优化性能（成立的前提是，每位用户都对数个电影进行了评价，并且每部电影都被数位用户评价过的情况下，这样才能重复这个迭代过程，来估计出 $\theta$ 和 $x$）：
        
        $$
        \theta → x → \theta → x → \theta → x → ...
        $$

1. 系统过滤算法优化，同时求解 $\theta$ 和 $x$：

    - 原始过程1，给出 $x^{(1)},...,x^{(n_m)}$ (和电影评分)，估计 $\theta^{(1)},...,\theta^{(n_u)}$
    ...
    - 原始过程2，给出 $\theta^{(1)},...,\theta^{(n_u)}$，估计 $x^{(1)},...,x^{(n_m)}$
    ...
    - 将两个过程合并，同时最小化 $\theta$ 和 $x$

    $$
    \underset{x^{(1)},...,x^{(n_m)},\theta^{(1)},...,\theta^{(n_u)}}{min}J(x^{(1)},...,x^{(n_m)},\theta^{(1)},...,\theta^{(n_u)})
    $$

    $$
    =\underset{x^{(1)},...,x^{(n_m)}, \theta^{(1)},...,\theta^{(n_u)}}{min}
    \frac{1}{2} 
    \sum_{(i,j):r(i,j)=1} \Big((\theta^{(j)})^Tx^{(i)}-y^{(i,j)}\Big)^2 + 
    \frac{\lambda}{2} 
    \sum_{i=1}^{n_m}
    \sum_{k=1}^{n}(x_k^{(i)})^2 + 
    \frac{\lambda}{2} 
...
1. 矢量化，低秩矩阵分解（Vectorization，Low rank matrix factorization）
    - 电影特征矩阵：
    ...
    - 此外，还可以通过计算电影特征距离的大小，来评估电影之间的相似性。
    ![alt](./mlpics/rs2.png)

    - 均值规范化（Mean normalization）
        - 一种实施方式。线上训练，自动化赋值与更新初始均值。（打过分的根据历史数据计算偏好并推荐，没打过分的就预测为均值）关心那些没评过分的用户，比起关心那些没有被评分的电影，要重要的多。
...
1. 大规模机器学习（适用大数据集的处理算法）
    - 一种获得高性能机器学习系统的途径：即采用低偏差的学习算法，并用大数据进行训练。
    ![alt](./mlpics/bigdataset.png)

    - 判断增加数据量是否对算法训练有帮助，仍通过学习曲线来判断
    ![alt](./mlpics/bigdataset1.png)

1. 随机梯度下降SGD（Stochastic gradient descent）
    - 传统思路：类似于线性回归、逻辑回归和神经网络之类的算法，推导算法的过程，是提出一个代价函数，或提出一个优化目标，然后使用梯度下降这样的算法，求代价函数的最小值。
    ![alt](./mlpics/sgd.png)
    
    - 但是当训练集很大时，梯度下降算法的计算量会变得非常大。随机梯度下降法便是以此生成的，对普通梯度下降法的改进算法，可以使得梯度下降法应用于更大的数据集中。
    ![alt](./mlpics/sgd1.png)

    - 批量梯度下降（Batch gradient descent），批量的意思是我们每次都要同时考虑所有的训练样本，被称作一批训练样本。
    - 随机梯度下降：在每次迭代中，不需要考虑所有的训练样本，仅仅只需要考虑一个训练样本。随机打乱输入样本，每输入一个样本就梯度下降一次，而不是遍历所有样本梯度下降一次。（对大数据样本更适用，小样本会容易过拟合）随机梯度下降不需要对全部m个样本求和来得到梯度项，而只需要对单个训练样本求出这个梯度项，并在这个过程中来优化参数。
    ![alt](./mlpics/sgd2.png)

    - 主要差异：在随机梯度下降中，每一次迭代都会更快，因为我们不需要对所有训练样本进行求和，每一次迭代只需要保证能拟合某一个训练样本就行了。训练优化过程可能非常曲折，但是最终能够非常接近全局最小值，这样就能得到一个很好的假设，能得到一个很接近全局最小值的参数，对于实际应用目的来说已经足够了。SGD降低了内层循环的次数。对于普通的梯度下降算法，每一小步却要遍历整个数据集，这也说明了为什么随机梯度下降法要快得多。
    ![alt](./mlpics/sgd4.png)


1. Mini-Batch梯度下降
    - 这种方法有时甚至比SGD速度还要快一点。在向量化的过程中，小批量样本的并行计算，总的时间成本会比SGD的单个样本，整体效率更高。
    ![alt](./mlpics/minibatch.png)


1. 随机梯度下降收敛（Stochastic gradient descent convergence）：
    - 收敛，不稳定，发散的集中状态。通过绘制代价函数和迭代次数的曲线来得到
    ![alt](./mlpics/sgd5.png)

    - 学习率 $\alpha$ 衰减：实际应用中，可以设置动态变换的学习率。随着迭代次数的增加，学习率逐渐降低，越接近最优解区域波动越来越小，直至最终收敛。
    ![alt](./mlpics/sgd6.png)


1. 在线学习
    - 一种新的大规模的机器学习机制。规模化机器学习系统，有持续连续(Streaming)的数据流


1. 减少映射和数据并行（MapReduce & data parallelism）
    - Map-reduce并行计算
    ![alt](./mlpics/mr.png)
    
    - 并行计算架构，多台计算机器（算法中求和的部分）
    ![alt](./mlpics/mr1.png)

    - 并行计算的另一种方式，一个机器多个核，多线程计算。关于多核机器并行计算，还需要提到一点，取决于不同的实现，如果有一个多核机器，然后有些线性代数的库，可以自动的在不同的核心上进行并行代数计算，而且学习算法有非常好的向量化表示，就可以直接以向量化的方式应用标准学习算法。
    ![alt](./mlpics/mr2.png)

    - 进程和线程的区别：进程是操作系统资源分配的基本单位，而线程是处理器任务调度和执行的基本单位。
        - 一个在内存中运行的应用程序。每个进程都有自己独立的一块内存空间，一个进程可以有多个线程，比如在Windows系统中，一个运行的xx.exe就是一个进程。
        - 进程中的一个执行任务（控制单元），负责当前进程中程序的执行。一个进程至少有一个线程，一个进程可以运行多个线程，多个线程可共享数据。

        ![alt](./mlpics/process.png)
...
1. OCR（Optical character recognition）
    - 目的：
        - 展示一个复杂的机器学习系统，是如何被组合起来的
        - 机器学习流水线的有关概念，以及如何分配资源，来对下一步计划作出决定
        - 机器学习中很多有用的想法和概念，比如如何将机器学习应用到计算机视觉问题中，人工数据合成等
    - OCR Pipeline（流水线/管道）：
        - 照片输入
        - 文字识别/text detection，从照片中锁定文字区域
        - 文字分割/character segmentation，将文字区域中的字母单独分类出来
        - 文字分类/character classification，将分离出的文字字母，识别为标准字母
        ![alt](./mlpics/ocr.png)


1. 滑动窗口分类器（sliding window classifier）
    - 目标分割和检测图块，要保证区域有相同的长款比。定义像素点a x b，定义图块每一步移动的距离（也称作步长-step size/slide parameter,步长比较常见的取4像素、8像素或更大的值）

1. 人工合成数据（artificial data synthesis）
    - 在增大数据样本之前，确保分类器是低偏差的
    - 获取更多数据的方法：
        - 人工合成数据（图片变换、翻转、扭曲、加入新的随机偏差）
        - 自己收集新数据/打标签
        - “众包”（crowd source） - 网上提供雇人的服务，以相对低廉的价格来标记大量的训练集，叫做“众包”（或众包数据标记）


1. 上限分析（ceiling analysis）
    - 分解主要步骤，并通过人工方法输入标准正确答案（遍历所有训练集，人工告诉算法该步骤正确的结果应该是什么），然后看整体效果提升的程度，可以看出哪一步是最值得花精力去优化及改进的。
    ![alt](./mlpics/cellinganalysis.png)

    - 以人脸识别的case为例，首先通过预处理去除背景，然后对面部特征（眼、鼻子、嘴）进行分割，最后根据分割的特征，结合逻辑回归算法，得到最后的标签结果。对每一步进行上限分析，并输出每一步改进到完美的程度，分别对最终预测结果提升的空间有多大。
    ![alt](./mlpics/cellinganalysis1.png)


1. 总结
    - 课程大纲知识点总结
    ![alt](./mlpics/summary.png)
